...
def verify_deep_transformer_stack():
    ...
    transformer_stack = nn.Sequential(
        *[RenormTransformerLayer(d_model=EMBED_DIM, n_heads=HEADS) for _ in range(NUM_BLOCKS)]
    )
    ...
    print("-" * 65)
    print(
        f"Block 0  - Functional Attention Beta : {torch.sigmoid(first_block.attn.beta_proxy).item():.6f}"
    )
    print(
        f"Block 0  - Functional MLP Beta       : {torch.sigmoid(first_block.beta_mlp_proxy).item():.6f}"
    )
    print(
        f"Block 11 - Functional Attention Beta : {torch.sigmoid(last_block.attn.beta_proxy).item():.6f}"
    )
    print(
        f"Block 11 - Functional MLP Beta       : {torch.sigmoid(last_block.beta_mlp_proxy).item():.6f}"
    )
    print("-" * 65)
