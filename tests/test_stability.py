...
def test_stability(depth=500, dim=64):
    ...
    standard_model = nn.Sequential(
        *[nn.Sequential(nn.Linear(dim, dim), nn.SiLU()) for _ in range(depth)]
    )
    ...
    s_var_str = (
        f"{s_var:.6f}"
        if not math.isnan(s_var) and not math.isinf(s_var)
        else "EXPLODED (NaN / Inf)"
    )
    ...
    if math.isnan(s_var) or math.isinf(s_var):
        print(
            "SUCCESS: Standard ResNet suffered total spectral breakdown. Renorm-Native stabilized signal flow."
        )
    else:
