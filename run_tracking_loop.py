...
def execute_drift_tracking():
    ...
    optimizer = optim.SGD(
        model.parameters(), lr=5.0
    )  # Cranked learning rate to test boundary stress

    initial_betas = torch.sigmoid(model.beta_proxy).detach().clone()
    print("-" * 75)
    print(
        f"{'Step':<6} | {'Layer ID':<10} | {'Current Beta':<14} | {'Total Shift Delta':<18} | {'Proxy Value':<12}"
    )
    print("-" * 75)
    ...
    for step in range(1, STEPS + 1):
        ...
        with torch.no_grad():
            ...
            for layer_idx in [0, DEPTH - 1]:
                ...
                print(
                    f"{step:<6} | {layer_label:<10} | {curr_beta:<14.6f} | {delta:<+18.6f} | {proxy_val:<12.4f}"
                )
        print("-" * 75)
